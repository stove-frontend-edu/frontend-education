...
function addImage(container, data) {
	const imageElement = document.createElement('img');
  imageElement.setAttribute('class', 'lazy-image');
  imageElement.setAttribute('src', 'https://cdn-icons-png.flaticon.com/512/456/456246.png');
    ...
    const imageIo = new IntersectionObserver((entries, observer) => {
        entries.forEach(entry => {
            ...
            if (entry.isIntersecting) {
                entry.target.src = imageData[+entry.target.dataset.index].src;
